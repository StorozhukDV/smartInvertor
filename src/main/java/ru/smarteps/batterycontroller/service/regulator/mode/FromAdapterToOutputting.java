    ...
    public OperationMode execute(double currentOutput, double Umeas, RegulatorSettings rs) {
        if (rs.getOperationMode() == OperationMode.Adapter ) {
            if (rs.getSAngle()!=-1) {
                ...
                if (timePassed) {
                    log.info("Changing mode from adapter to power output.");
